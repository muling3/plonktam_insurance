...
const providers: { [k: string]: { cat: string; cost: string } } = {
  "1": {
    cat: "Ksh. 500, 000 To 1M",
    cost: "45, 000",
  },
  "2": {
    cat: "Ksh. 1M To Ksh. 2.5M",
    cost: "67, 000",
  },
  "3": {
    cat: "Over Ksh. 2.5M",
    cost: "Request a promotion",
  },
};
const Page = ({ params: { val } }: { params: { val: string } }) => {
  const category = "Comprehensive Covers";
  const insurance = "Jubilee";
  ...
  if (!provider) {
    return (
      <div className="w-full flex-1 bg-slate-200 flex justify-center items-center flex-col  px-2 py-2 sm:px-24 sm:py-4">
        ...
        <Link
          href={"/service/privateVehicles/comprehensive/jubilee"}
        ...
        <div className={`bg-slate-300 px-2 py-2 sm:px-32 sm:py-4`}>
          <div className="header w-full py-1 flex justify-between items-center">
            <Link href={"/service/privateVehicles/comprehensive/jubilee"}>
              <span className="material-symbols-outlined p-2 rounded-full border border-gray-500 text-gray-950">
                close
              </span>
            </Link>
